...
# fftw3
if [ ! -d ${REPO_DIR}/fftw3_${WN} ]; then
  cd ${REPO_DIR} && rm -f fftw-3.3.10.tar.gz && rm -rf fftw3_${WN}
  cd ${REPO_DIR} && wget https://www.fftw.org/fftw-3.3.10.tar.gz && tar xf fftw-3.3.10.tar.gz
  mv fftw-3.3.10 fftw3_${WN}
  echo -e "\n\n********************************************************"
  echo "start build of fftw3_${WN}"
  cd ${REPO_DIR}/fftw3_${WN} && ./bootstrap.sh && \
    CC=${CROSS}-gcc \
    AR=${CROSS}-ar \
    RANLIB=${CROSS}-ranlib \
    ./configure --prefix=${REPO_DIR}/mingw_fftw3_${WN} --host=${CROSS} --disable-shared && \
    make && make install
    echo -e "\n\nlisting of ${REPO_DIR}/mingw_fftw3_${WN}"
    ls -alh ${REPO_DIR}/mingw_fftw3_${WN}
    echo -e "\nlisting of ${REPO_DIR}/mingw_fftw3_${WN}/include"
    ls -alh ${REPO_DIR}/mingw_fftw3_${WN}/include
    echo -e "\nlisting of ${REPO_DIR}/mingw_fftw3_${WN}/lib"
    ls -alh ${REPO_DIR}/mingw_fftw3_${WN}/lib
    echo -e "\n"
fi
...
if /bin/true; then
  ...
  mkdir ${REPO_DIR}/build_${WN} && cd ${REPO_DIR}/build_${WN} && \
    cmake -DCMAKE_TOOLCHAIN_FILE=${REPO_DIR}/${TOOLCHAIN} \
      ...
      -DFFTW3_INCLUDE_DIR=${REPO_DIR}/mingw_fftw3_${WN}/include \
      -DFFTW3_LIBRARIES=${REPO_DIR}/mingw_fftw3_${WN}/lib/libfftw3.a \
      ../  && \
    make && make install
