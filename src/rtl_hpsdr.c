      ...
      if (!using_IQfiles) {
        if ((mcb.rcb[rcb->rcvr_num].new_freq) && (mcb.rcb[rcb->rcvr_num].new_freq != 10000000)) {
           ...
           if ((((((mcb.freq_ttime[rcb->rcvr_num].time * 1000) +
            ...
            else ("RRK: set rcvr %d to freq %d with offset %+d\n",
                   mcb.rcb[rcb->rcvr_num].rcvr_num + 1,
                   mcb.rcb[rcb->rcvr_num].new_freq,
                   mcb.freq_offset[rcb->rcvr_num]);
...
int main (int argc, char *argv[]) {
  ...
  struct sigaction sigact;
   ...
   mcb.output_rate = 48000;
   mcb.serialstr[0] = 0;
   mcb.signal_multiplier = 1; //RRK 50;
   mcb.cal_state = CAL_STATE_0;
   mcb.calibrate = 0;
   mcb.up_xtal = 0;
...
void process_ep2_2(uint8_t *frame) {
  int rc;
  int mod;
  int i, num_rcvrs;
  ...
  case 1:
    ...
    chk_data(((frame[4] >> 3) & 7) + 1, receivers, "RECEIVERS");
                      if (last_num_rcvrs != receivers) {
                         last_num_rcvrs = receivers;
                         num_rcvrs = last_num_rcvrs;
    ...
    chk_data(((frame[4] >> 7) & 1), common_freq, "Common Mercury Freq");
    mod = 0;
    rc = frame[3] & 0x03;
